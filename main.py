...
# https://github.com/Ronoh4/Single-Turn-Multi-Function-Agents
...
scope = spark.conf.get("spark.databricks.clusterUsageTags.clusterName").split('_')[0]
os.environ["OPENAI_API_BASE"] = dbutils.secrets.get(scope=scope, key="OPENAI_URL")
os.environ["OPENAI_API_KEY"] = dbutils.secrets.get(scope=scope, key="OPENAI_API_KEY")
    ...
    if True:
        try:
            ...
            data = [{
                ...
                "portOfOrigin": "Toronto",
                "portOfOriginCode": 202,
                "countryOfDestination": "China",
                "portOfDestination": "Guangzhou",
                "portOfDestinationCode": "Port2",
                "carrier": "UPS",
                "generalCargo": 1,
                "validFrom": "10/01/2020",
                "validTo": "10/02/2020",
                "terms": "FOB",
                "bookingLink": "Link"
            },
    ...
    if True:
        try:
            ...
            data = [{
                "quoteId": 100,
                "countryOfOrigin": "Canada",
                "originCityAirport": "Toronto",
                "airportOfLoading": "MSP",
                "airportOfOriginCode": 212,
                "countryOfDestination": "Zimbabwe",
                "destinationCityAirport": "Mogadishu",
                "airportOfDischarge": "Sierra Leone",
                "airportOfDischargeCode": 555,
                "cargoType": "general",
                "carrier": "UPS",
                "carrierCode": 111,
                "travelTime": 12,
                "offerValidUntil": "Forever",
                "termsConditions": "FDB",
                "bookingLink": "link"
            },
                {
...
def get_rag_response(query):
    parser = LlamaParse(
        api_key="llx-YE1soZpIFUr8xOPWB9NfLmXLDBhrlNpQaNfsBKl5rNlFDSN5",
        result_type="text",
        language="en",
        varbose=True
    )
...
openai_key = openai_api_key  # JDM: Not sure if this is correct.  I didn't see where the guy used the openai_api_key above
llm = OpenAI(api_key=openai_key, model="gpt-3.5-turbo")

agent = OpenAIAgent.from_tools([rag_tool, LcL_freight_tool, FcL_freight_tool, Air_freight_tool, RoRo_freight_tool],
                               llm=llm, verbose=True)

response = agent.chat(
    "What is the age restriction for car that can be imported into Kenya? and What are the freight rates for RoRo freight cargo from Country Japan to Country Kenya for an Audi model a5 coupe for fuel type petrol and weighing 1400 for drive type 2wd?")
